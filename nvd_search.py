...
def retry_with_backoff(func, config: Optional[NVDConfig] = None):
    """
...
def search_nvd(
    ...
    def _search_cves():
        try:
            ...
            raise NVDAPIError(f"Timeout while searching CVEs for {cpe_name}")
        except HTTPError as e:
            if e.response.status_code == 429:
                raise NVDAPIError("Rate limit exceeded. Please wait before making more requests.")
            raise NVDAPIError(f"HTTP error while searching CVEs: {str(e)}")
        except Exception as e:
            raise NVDAPIError(f"Unexpected error while searching CVEs: {str(e)}")
